...
void main() {
  ...
  group('Moteur de Routage - Safe Path Engine', () {
    test(
        'Calcul de trajectoire : Les mouvements diagonaux doivent être fortement pénalisés',
        () {
      ...
      expect(coutDiagonal, greaterThan(coutLineaire * 1.414),
          reason:
              'La diagonale doit être plus pénalisée que le ratio géométrique standard pour éviter les bâtiments.');
    });
  });

  group('Persistence & Sécurité - Hive Database', () {
    test(
        'Gestion du Time-To-Live (TTL) : Les alertes de plus de 24h doivent être purgées',
        () {
      ...
      expect(estExpiree(alerteValide), isFalse,
          reason: 'Une alerte de 12h doit rester active.');
      expect(estExpiree(alerteObsolete), isTrue,
          reason:
              'Une alerte de 25h doit être automatiquement éligible à la purge.');
    });
  });

  group('Réseau P2P - Consensus de la Ruche', () {
    test(
        'Validation locale : Un minimum de 3 confirmations est requis pour le consensus',
        () {
      ...
      expect(verifierConsensus(confirmationsAlerte), isFalse,
          reason:
              'Le consensus ne doit pas être atteint avec seulement 2 validations.');

      confirmationsAlerte = 3;
      expect(verifierConsensus(confirmationsAlerte), isTrue,
          reason:
              'Le consensus doit être validé dès la 3ème confirmation locale.');
    });
  });
}
