...
/**
 * Retourne uniquement les coordonnées des zones dangereuses validées
 * (utilisées par events_manager pour le calcul de Safe Route).
 *
 * @returns {Array<{lat:number, lon:number, type:string, radius_m:number}>}
 */
function getValidatedDangerZones() {
  ...
  for (const entry of reportStore.values()) {
    ...
    // Rayon d'influence selon le type
    let radius = 80;
    if (entry.type === 'barrage') radius = 150;
    if (entry.type === 'casseurs') radius = 120;
    if (entry.type === 'danger_collectif') radius = 200;

    zones.push({ lat: entry.lat, lon: entry.lon, type: entry.type, radius_m: radius });
  }
  return zones;
}
