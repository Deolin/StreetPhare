...
const EVENTS_CATALOG = [
  ...
  },
  {
    ...
    description:
      ...
      'pour les familles et les enfants lors des grandes manifestations.',
 ...
 * Exclut les arêtes dont un segment traverse une zone dangereuse.
 *
 * @param {Array<{lat,lon,radius_m?}>} dangerZones - zones à éviter
 * @returns {Map<string, Array<{to, dist, blocked}>>}
 */
function buildAdjacency(dangerZones = []) {
  ...
  for (const edge of PEDESTRIAN_GRAPH.edges) {
    ...
    // Vérification : l'arête traverse-t-elle une zone de danger ?
    const midLat = (nFrom.lat + nTo.lat) / 2;
    const midLon = (nFrom.lon + nTo.lon) / 2;
    const blocked = dangerZones.some(z => {
      const r = z.radius_m || 80;
      return haversineM(midLat, midLon, z.lat, z.lon) < r;
    });

    adj.get(edge.from).push({ to: edge.to,   dist: edge.dist, blocked });
    adj.get(edge.to).push(  { to: edge.from, dist: edge.dist, blocked });
  }
  return adj;
}
 ...
 * @param {boolean} avoidBlocked - si true, ignore les segments bloqués
 */
function aStar(startId, goalId, adj, avoidBlocked = true) {
...
async function computeSafeRoutes(eventId, dangerZones = [], fromOverride = null) {
  ...
  // Itinéraire principal (évite les dangers)
  const mainPath = aStar(startId, goalId, adj, true)
    || aStar(startId, goalId, adjUnsafe, false); // fallback sans contrainte
  ...
  routes.push(buildRouteResult(
    ...
    mainPath.totalDist,
    Math.round(mainPath.totalDist / 1.2),   // ~1.2 m/s marche lente
  ...
  for (let i = 0; i < 3; i++) {
    ...
    const altPath = aStar(startId, goalId, adjAlt, true)
      || aStar(startId, goalId, adjUnsafe, false);
    ...
    if (altPath && altPath.nodes.join(',') !== mainPath.nodes.join(',')) {
      routes.push(buildRouteResult(
        ...
        altPath.totalDist,
        Math.round(altPath.totalDist / 1.2),
...
function buildAdjacencyExcluding(dangerZones, excludedNodes = []) {
  ...
  for (const edge of PEDESTRIAN_GRAPH.edges) {
    ...
    adj.get(edge.from) && adj.get(edge.from).push({ to: edge.to,   dist: edge.dist, blocked });
    adj.get(edge.to)   && adj.get(edge.to).push(  { to: edge.from, dist: edge.dist, blocked });
  }
  return adj;
}
...
function generateAltRoutes(startCoords, endCoords, dangerZones, waypoints) {
  ...
  for (let i = 0; i < 3; i++) {
    ...
    const leg1 = aStar(startId, midId, adj, true) || aStar(startId, midId, adj0, false);
    const leg2 = aStar(midId,   goalId, adj, true) || aStar(midId, goalId, adj0, false);

    if (leg1 && leg2) {
      const nodes = [...leg1.nodes, ...leg2.nodes.slice(1)];
      const dist  = leg1.totalDist + leg2.totalDist;
